...
A new user agent is created via the `SIP.UA` constructor.  There are no mandatory parameters for creating a new user agent, although most applications will define at least [`uri`](/api/0.5.0/ua_configuration_parameters/#uri) and [`ws_servers`](/api/0.5.0/ua_configuration_parameters/#ws_servers). Check the full list for optional [UA Configuration Parameters](/api/0.5.0/ua_configuration_parameters/).
